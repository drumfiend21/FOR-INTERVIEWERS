...
describe('For a given promiseA (pA)', function(){
  ...
  it('.then can be chained many times', function(){
    ...
    //   test = data;
    // });
      console.log("THEN TESTING")
      var promiseB = promiseA.then( add1 )
      console.log("promise B",promiseB)
      var promiseC = promiseB.then( add1 )
      console.log("promise C", promiseC)
      var promiseD = promiseC.then()
      console.log("promise D", promiseD)
      var promiseE = promiseD.then(function(data){ test = data})
